...
const App = () => {
  ...
  const handlePredict = async () => {
    ...
    const resultWithMeta = { ...data, fileName: selectedFile.name };
    setResult(resultWithMeta);
    setHistory([resultWithMeta, ...history].slice(0, 3));
    localStorage.setItem('uem_lung_history', JSON.stringify([resultWithMeta, ...history].slice(0, 3)));
    setLoading(false);
  };
